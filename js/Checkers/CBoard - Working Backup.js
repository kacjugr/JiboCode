...
//require(["jquery"], function ($) {
	...
	resetState = function() {
		...
		console.log("resetting total_table_height");
		...
		$('body').animate({backgroundColor: '#d0e4fe'}, 'slow');
	...
	pauseGame = function() {
		...
		// Pause the audio
		//document.getElementById('walkAudio').pause();
	...
	var player_last_tile = player_tile;
	...
	runGame = function() {
		...
		if(newFrame(new Date())) {
			...
			//console.log(facing);
			player_last_tile.row = player_tile.row;
			player_last_tile.column = player_tile.column;
			if(facing === 0) {
				player_tile.row--;		// UP
			} else if(facing === 1) {
			...
			if(player_tile.row < 0 || player_tile.row >= the_board.length || player_tile.column < 0 || player_tile.column >= the_board[0].length) {
				...
				$('body').animate({backgroundColor: '#66FF99'}, 'slow');
	...
	loadCheckersZone = function() {
		...
		checkers_play.append("<button id='startGame' >Start</button>");					// 'Init Board' button
		checkers_play.append("<button id='pauseGame' >Pause</button>");					// 'Shuffle Board' button
		checkers_play.append("<button id='resumeGame'>Resume</button><br><br>");		// 'Play' button
/*
...
*/
	...
	buildTable = function() {
		...
		//console.log(game_table_obj.children().length);
		//console.log(document.getElementById('gameTable').style.offsetheight);
				...
				if(total_table_height === -1)
				{
					...
					$(document).ready(function() {
						$(arrow_name).load(function() {
							...
							// When rescaling is done, slide down the panel
//							console.log(document.getElementById("gameBoard"));
//							console.log("ready to scroll down pane");
						});
					});
				}
			}
